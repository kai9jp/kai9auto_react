...
const CustomTable: React.FC<CustomTableProps> = (props:CustomTableProps) => {
    ...
    //ドラッグイベント
    const onDrag = () => {
        calcCardFooterBottom(isMinimized);//フッター座標の反映
        ...
        if (props.ComponentPositionsID){
            if (rndRef.current) {
                ...
                if (selfElement) {
                    // 要素の座標を取得
                    const { x, y } = selfElement.getBoundingClientRect();
                    // 親要素を取得
                    ...
                    if (parentElement) {
                        ...
                        // setPosition()を使用して設定
                        setPosition(selfElement.clientWidth, selfElement.clientHeight, relativeX, relativeY);
                    }
                }
            }
        }
